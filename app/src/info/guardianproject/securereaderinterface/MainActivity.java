...
public class MainActivity extends ItemExpandActivity implements OnSharedPreferenceChangeListener
{
	...
	private void addUICallbackListener()
	{
		mCallbackListener = new UICallbackListener()
		{
			...
			@Override
			public boolean onCommand(int command, Bundle commandParameters)
			{
				switch (command)
				{
				...
				{	
					...
					{
						MediaContent mc = (MediaContent) commandParameters.getSerializable("media");
						if ((mc.getType() != null && (mc.getType().startsWith("audio") || mc.getType().startsWith("video")))
								|| "audio".equals(mc.getMedium())) 
						{
							if (getMusicPlayerInterface() != null)
							{
								try
								{
									getMusicPlayerInterface().playWithId(mc.getDatabaseId());
								}
							...
							return true; //Dont do default handling
						}
					}
					break;
				}
			...
			case FAVORITES:
			{
				...
				if (items.size() == 0)
				{
					...
					if (allSubscribed != null)
					{
						...
						items.addAll(allSubscribedItems.subList(0, Math.min(5, allSubscribedItems.size())));
					}
				}

				boolean shouldShowAddFavoriteHint = fakedItems || items == null || items.size() == 0;
				mStoryListView.updateItems(mContext, items, shouldShowAddFavoriteHint ? R.layout.story_list_hint_add_favorite : 0, mIsUpdate);
				break;
			}
	...
	public void updatePlaylist(ArrayList<Item> items)
	{
		if (getMusicPlayerInterface() != null)
		{
			try
			{
				getMusicPlayerInterface().clearPlaylist();

				for (Item item : items)
				{
					if (item.getNumberOfMediaContent() > 0)
					{
						for (MediaContent mc : item.getMediaContent())
						{
							if ((mc.getType() != null && (mc.getType().startsWith("audio") || mc.getType().startsWith("video")))
									|| "audio".equals(mc.getMedium()))
							{
//								if (socialReader.isMediaContentLoaded(mc))
								{
									getMusicPlayerInterface().addToPlaylist(new ParcelableMediaContent(mc));
								}
							}
						}
					}
				...
				if (getMusicPlayerInterface().isPaused())
					; // do nothing getMusicPlayerInterface().resume();
				else if (!getMusicPlayerInterface().isPlaying())
					getMusicPlayerInterface().play(0);
			}
			catch (RemoteException e)
			{
			}
