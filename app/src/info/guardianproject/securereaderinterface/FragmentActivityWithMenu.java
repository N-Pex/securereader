...
import info.guardianproject.securereaderinterface.musicplayer.MusicPlayerInterface;
import info.guardianproject.securereaderinterface.musicplayer.MusicPlayerService;
...
public class FragmentActivityWithMenu extends LockableActivity implements LeftSideMenuListener, FeedFilterViewCallbacks
{
	...
	public void onMenuCreated(final View parent, final View menuRoot, final View menu)
	{
		...
		mMenuViewHolder.playingNow.btnPlayPause.setOnClickListener(mMediaPlayerButtonClicked);
		mMenuViewHolder.playingNow.btnPrevious.setOnClickListener(mMediaPlayerButtonClicked);
		mMenuViewHolder.playingNow.btnNext.setOnClickListener(mMediaPlayerButtonClicked);
		mMenuViewHolder.playingNow.btnStop.setOnClickListener(mMediaPlayerButtonClicked);
	}
	...
	private class MenuViewHolder
	{
		...
		public MediaPlayerViewHolder playingNow;
	}

	private class MediaPlayerViewHolder
	{
	...
	private void createMenuViewHolder()
	{
		if (mMenuViewHolder == null)
		{
			...
			View playingNow = menuView.findViewById(R.id.playingNow);
			MediaPlayerViewHolder vh = new MediaPlayerViewHolder();
			vh.tvTitle = (TextView) playingNow.findViewById(R.id.tvTitle);
			vh.tvSubTitle = (TextView) playingNow.findViewById(R.id.tvSubTitle);
			vh.btnPlayPause = (ImageView) playingNow.findViewById(R.id.ivPlayPause);
			vh.btnPrevious = (ImageView) playingNow.findViewById(R.id.ivMediaPrevious);
			vh.btnNext = (ImageView) playingNow.findViewById(R.id.ivMediaNext);
			vh.btnStop = (ImageView) playingNow.findViewById(R.id.ivStop);
			vh.pbPosition = (ProgressBar) playingNow.findViewById(R.id.pbPosition);
			vh.tvCurrentPosition = (TextView) playingNow.findViewById(R.id.tvCurrentPosition);
			vh.tvDuration = (TextView) playingNow.findViewById(R.id.tvDuration);
			mMenuViewHolder.playingNow = vh;
		}
	}
	...
	private ServiceConnection mConnection = new ServiceConnection()
	{
		...
		{
			mMusicPlayerInterface = MusicPlayerInterface.Stub.asInterface((IBinder)service);
			IntentFilter intentFilter = new IntentFilter(MusicPlayerService.BROADCAST_ACTION_PROGRESS);
			intentFilter.addAction(MusicPlayerService.BROADCAST_ACTION_DURATION);
			intentFilter.addAction(MusicPlayerService.BROADCAST_ACTION_STATUS);
			intentFilter.addAction(MusicPlayerService.BROADCAST_ACTION_TRACK_INFO);
			LocalBroadcastManager.getInstance(FragmentActivityWithMenu.this).registerReceiver(mMusicPlayerReceiver, intentFilter);
		}

		public void onServiceDisconnected(ComponentName className)
		{
			mMusicPlayerInterface = null;
			LocalBroadcastManager.getInstance(FragmentActivityWithMenu.this).unregisterReceiver(mMusicPlayerReceiver);
		}
	};
	
	protected MusicPlayerInterface getMusicPlayerInterface()
	{
		return mMusicPlayerInterface;
	}
	...
	private BroadcastReceiver mMusicPlayerReceiver = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			if (MusicPlayerService.BROADCAST_ACTION_PROGRESS.equals(intent.getAction()))
			{
				int progress = intent.getIntExtra(MusicPlayerService.EXTRA_PROGRESS, -1);
				mMenuViewHolder.playingNow.tvCurrentPosition.setText(formatSeconds(progress));
				mMenuViewHolder.playingNow.pbPosition.setProgress(progress);
			}
			else if (MusicPlayerService.BROADCAST_ACTION_DURATION.equals(intent.getAction()))
			{
				int duration = intent.getIntExtra(MusicPlayerService.EXTRA_DURATION, -1);
				mMenuViewHolder.playingNow.tvDuration.setText(formatSeconds(duration));
				mMenuViewHolder.playingNow.pbPosition.setMax(duration);
			}
			else if (MusicPlayerService.BROADCAST_ACTION_TRACK_INFO.equals(intent.getAction()))
			{
				String track = intent.getStringExtra(MusicPlayerService.EXTRA_TRACK);
				String artist = intent.getStringExtra(MusicPlayerService.EXTRA_ARTIST);
				
				mMenuViewHolder.playingNow.tvTitle.setText(track);
				mMenuViewHolder.playingNow.tvSubTitle.setText(artist);
				mMenuViewHolder.playingNow.tvCurrentPosition.setText("");
				mMenuViewHolder.playingNow.tvDuration.setText("");
				mMenuViewHolder.playingNow.pbPosition.setMax(0);
				mMenuViewHolder.playingNow.pbPosition.setProgress(0);
			}
			else if (MusicPlayerService.BROADCAST_ACTION_STATUS.equals(intent.getAction()))
			{
				int status = intent.getIntExtra(MusicPlayerService.EXTRA_STATUS, 0);
				switch (status)
				{
				case MusicPlayerService.STATUS_IDLE:
					mMenuViewHolder.playingNow.btnPlayPause.setImageResource(R.drawable.ic_song_play);
					break;
				case MusicPlayerService.STATUS_PAUSED:
					mMenuViewHolder.playingNow.btnPlayPause.setImageResource(R.drawable.ic_song_play);
					break;
				case MusicPlayerService.STATUS_PLAYING:
					mMenuViewHolder.playingNow.btnPlayPause.setImageResource(R.drawable.ic_song_pause);
					break;
				case MusicPlayerService.STATUS_LOADING:
					mMenuViewHolder.playingNow.tvTitle.setText(R.string.music_player_loading);
					mMenuViewHolder.playingNow.tvSubTitle.setText("");
					break;
				}
			}
		}
	...
	private View.OnClickListener mMediaPlayerButtonClicked = new View.OnClickListener()
	{
		@Override
		public void onClick(View v)
		{
			if (getMusicPlayerInterface() != null)
			{
				try
				{
					if (v == mMenuViewHolder.playingNow.btnPlayPause)
					{
						if (getMusicPlayerInterface().isPaused())
							getMusicPlayerInterface().resume();
						else if (getMusicPlayerInterface().isPlaying())
							getMusicPlayerInterface().pause();
					}
					else if (v == mMenuViewHolder.playingNow.btnPrevious)
					{
						getMusicPlayerInterface().previous();
					}
					else if (v == mMenuViewHolder.playingNow.btnNext)
					{
						getMusicPlayerInterface().next();
					}
					else if (v == mMenuViewHolder.playingNow.btnStop)
					{
						getMusicPlayerInterface().stop();
					}
				}
